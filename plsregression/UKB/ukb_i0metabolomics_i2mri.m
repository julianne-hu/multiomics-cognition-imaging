...
i0age=merged_data.p21003_i0; i2age=merged_data.p21003_i2; sex=merged_data.p31; site=merged_data.p54_i2; motion=merged_data.p24419_i2; icv=merged_data.p26521_i2; 
merged_data.delta_time=i2age-i0age; delta_time=merged_data.delta_time;
...
naninx=any(isnan(Y),2) | any(isnan(X), 2) | any(ismissing(site),2) | any(isnan(motion), 2); %|sum(ismissing(sex)')' | sum(ismissing(icv)')' | sum(ismissing(i0age)')' | sum(ismissing(i2age)')' | sum(ismissing(delta_time)')'
Y=Y(~naninx,:);
X=X(~naninx,:); 
i0age=i0age(~naninx); 
i2age=i2age(~naninx); 
...
clear x; 
for i=1:length(X(1,:)); 
    mdl = fitlm(table(i0age, delta_time, sex, site, X(:,i))); 
    x(:,i)=mdl.Residuals.Raw;
end
clear y; 
for i=1:length(Y(1,:)); 
    mdl = fitlm(table(i2age, delta_time, sex, site, motion, icv, Y(:,i))); 
...
writetable(PLS_table, '1PLS_metabolite_weights_i0metabolomics_i2mri_no_age.csv');
...
%SCATTER PLOT FOR Docosahexaenoic acid and
i = find(strcmp(met_names, 'Docosahexaenoic acid'))
x_val = x(:,i)
y = Y(:,1)
figure;
dscatter(x_val, y);
...
%%
i = find(strcmp(met_names, 'Phospholipids in VLDL'))
x_val = x(:,i);
