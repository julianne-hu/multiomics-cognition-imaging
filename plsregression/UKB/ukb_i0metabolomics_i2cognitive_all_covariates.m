...
sex=categorical(merged_data.p31); 
site=categorical(merged_data.p54_i2);
bmi=merged_data.x21001_0_0;
bmi(bmi < 0) = NaN;
education=merged_data.x6138_0_0;
education(education < 0) = NaN;
cvd_cols = merged_data(:, 382:535);
...
valid_bmi = ~isnan(bmi);
valid_education = ~isnan(education);
valid_ses = ~isnan(ses);
naninx = valid_Y & valid_sex & valid_site & valid_bmi & valid_education & valid_ses;
Y=Y(naninx,:); 
X=X(naninx,:); 
i0age=i0age(naninx); 
i2age=i2age(naninx); 
