...
rows_keep = ~any(isnan(X),2) & ~any(isnan(Y),2) & ~isnan(age) & ~isnan(sex) & ~isnan(education) & ~isnan(cvd) & ~isnan(bmi);
X=X(rows_keep, :);
Y=Y(rows_keep, :);
age=age(rows_keep);
sex=sex(rows_keep);
...
age_by_sex = age .* sex_num;

% Regress out age, sex, and site (covariates)
mdl = fitlm(table(age, sex, site, age_squared, age_by_sex, bmi, education, cvd, Y(:,1)), 'CategoricalVars',[2 3]); Y(:,1)=mdl.Residuals.Raw;
mdl = fitlm(table(age, sex, site, age_squared, age_by_sex, bmi, education, cvd, Y(:,2)), 'CategoricalVars',[2 3]); Y(:,2)=mdl.Residuals.Raw;
%mdl = fitlm(table(age, sex, site,Y(:,3)) ); Y(:,3)=mdl.Residuals.Raw;
%mdl = fitlm(table(i2age, sex, site,Y(:,4)) ); Y(:,4)=mdl.Residuals.Raw;
clear x; 
for i=1:length(X(1,:)) 
    mdl = fitlm(table(age, sex, site, age_squared, age_by_sex, bmi, education, cvd, X(:,i)), 'CategoricalVars', [2 3]); 
