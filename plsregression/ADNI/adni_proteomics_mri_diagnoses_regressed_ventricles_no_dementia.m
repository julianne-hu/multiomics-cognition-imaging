...
%sex=sex(naninx==0);
%site=site(naninx==0); 
rows_keep = ~any(ismissing([X Y age_pro age_mri dx_pro dx_mri delta_time_num sex site]), 2);
X=X(rows_keep, :);
Y=Y(rows_keep, :);
...
clear x; 
for i=1:length(X(1,:)) 
    mdl = fitlm(table(delta_time_num, dx_pro, sex, site,X(:,i))); 
    x(:,i)=mdl.Residuals.Raw;
end

clear y; 
for i=1:length(Y(1,:)); 
    mdl = fitlm(table(delta_time_num, dx_mri, sex, site, icv, Y(:,i))); 
