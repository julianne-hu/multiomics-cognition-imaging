...
%%% Removing NaNs
%naninx=sum(isnan(Y)')'; 
naninx=sum(isnan(Y)')' | sum(isnan(X)')';
Y=Y(naninx==0,:); 
X=X(naninx==0,:); 
...
% Flip direciton of trail making test (higher score = worse performance)
Y(:,1)=Y(:,1)*-1;
...
% 
 %figure;
 %hold on;
 % highlight inflammation proteins (red)
 %scatter(stats.W(is_inflammation,3), -log10(p_pls3(is_inflammation)), 25, 'MarkerFaceColor', [0.85 0.10 0.10], 'MarkerEdgeColor', 'none');
 %hold on;
 % highlight inflammation II proteins (red)
 %scatter(stats.W(is_inflammation_2,3), -log10(p_pls3(is_inflammation_2)), 25, 'MarkerFaceColor', [0.85 0.10 0.10], 'MarkerEdgeColor', 'none'); 
 %hold on;
 % highlight oncology proteins (pink)
 %scatter(stats.W(is_oncology,3), -log10(p_pls3(is_oncology)), 25, 'MarkerFaceColor', [0.80 0.00 0.70], 'MarkerEdgeColor', 'none');
 %hold on;
 % highlight oncology II proteins (pink)
 %scatter(stats.W(is_oncology_2,3), -log10(p_pls3(is_oncology_2)), 25, 'MarkerFaceColor', [0.80 0.00 0.70], 'MarkerEdgeColor', 'none');
 %hold on;
 % highlight cardiometabolic proteins (blue)
 %scatter(stats.W(is_cardiometabolic,3), -log10(p_pls3(is_cardiometabolic)), 25, 'MarkerFaceColor', [0.10 0.30 0.85], 'MarkerEdgeColor', 'none'); 
 %hold on;
 %% highlight cardiometabolic II proteins (blue)
 %scatter(stats.W(is_cardiometabolic_2,3), -log10(p_pls3(is_cardiometabolic_2)), 25, 'MarkerFaceColor', [0.10 0.30 0.85], 'MarkerEdgeColor', 'none'); 
 %hold on;
 % highlight neurology proteins (green)
 %scatter(stats.W(is_neurology,3), -log10(p_pls3(is_neurology)), 25, 'MarkerFaceColor', [0.00 0.60 0.20], 'MarkerEdgeColor', 'none');
 %hold on;
 % highlight neurology II proteins (green)
 %scatter(stats.W(is_neurology_2,3), -log10(p_pls3(is_neurology_2)), 25, 'MarkerFaceColor', [0.00 0.60 0.20], 'MarkerEdgeColor', 'none'); 
% 
...
%VOLCANO PLOTS FOR PLS 2 WITH TRANSPARENCY FOR NON SIG
 p_pls2 = (1 - normcdf(abs(plsweights2)));
% Define significance threshold
sig_threshold = (1 - normcdf(3));
